        ...
        public List<CountryModel> GetAll()
        {
            var data = from a in this._countryRepository.Table
                       select new CountryModel
                       {
                           ...
                           DomainId = a.DomainId,
                           IsActive = a.IsActive,
						   NationalLanguageId = a.CountryLanguages.Where(w => w.IsNationalLanguage == true).Select(s => s.LanguageId).FirstOrDefault(),
						   NationalLanguageName = a.CountryLanguages.Where(w => w.IsNationalLanguage == true).Select(s => s.Language.Name).FirstOrDefault(),
						   OtherLanguages = a.CountryLanguages.Where(w=>w.IsNationalLanguage==false).Select(s=> new CountryLanguageModel
						  {
							  Id = s.Id,
							  CountryId = s.CountryId,
							  LanguageId = s.LanguageId,
							  LanguageName=s.Language.Name,
							  IsDefaultLanguage = s.IsDefaultLanguage,
							  IsNationalLanguage = s.IsNationalLanguage
						  }).ToList()
					   };
